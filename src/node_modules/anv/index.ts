...
export interface MirrorFacet extends BaseFacet {
  ...
  tiers?: string[];
  media(metaMediaPage: string, tier: string, url: string): string | MirrorResult;
...
export interface GenericResolverFacet extends BaseFacet {
  ...
  description?: string;
  weight?: number;
  resolve(url: string, done: (err: string, resource: any) => void, options?: {}): void;
}
...
export interface StreamResolverFacet extends BaseFacet {
  ...
  resolve(url: string, bytes: number, out: (NodeJS.WritableStream & StreamOut) | string, info?: (data: {size?: number; bytes?: number; speed?: number; finished?: boolean}) => void, options?: StreamOptions): {
    stop(): void;
  }
}
